...
export const paramsIdSchema = Joi.object({
  id: joiCommon.joiString.required(),
}).options({
  abortEarly: false,
});

export const removeUserSubscriptionSchema = Joi.object({
  user_ids: joiCommon.joiString.required(),
  organization_id: joiCommon.joiString.required(),
  isRemove: joiCommon.joiBoolean.optional(),
});

export const paramsSlugSchema = Joi.object({
  common_id: joiCommon.joiString,
  isDashboard: joiCommon.joiBoolean,
  slug: joiCommon.joiString,
  showAll: Joi.boolean().optional(),
}).options({
  abortEarly: false,
});

export const paramsSlugSchemaPDF = Joi.object({
  order_slug: joiCommon.joiString,
  templateLanguage: joiCommon.joiString,
}).options({
  abortEarly: false,
});
